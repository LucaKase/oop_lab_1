...
class Table:
    ...
    def filter(self, condition):
        ...
        for item in self.table:
            if condition(item):
                temps.append(item)
        return Table("FilteredList", temps)
    ...
    def join(self, otherTable, joinParameter):
        # so i can have a completly new list of dictionaries, because otherwise i would also edit the my_table1, because it assigns the reference
        newTable = [dict(row) for row in self.table]
        for city in newTable:
            for country in otherTable.table:
                if city[joinParameter] == country[joinParameter]:
                    city['population'] = country['population']
                    city['EU'] = country['EU']
                    city['coastline'] = country['coastline']
        return Table(joinParameter, newTable)
...
print(my_table2.filter(lambda x: x['coastline'] == 'yes').aggregate(
